...
{
    printf("PRINTF: New udp stream");
    VLOG_WARN("VLOG: new udp stream\n");
    return new_fd_stream(name, fd, connect_status, AF_INET, streamp);
}
...
udp_open(const char *name, char *suffix, struct stream **streamp, uint8_t dscp) 
{
   ...
   int fd_udp, error;
   error = inet_open_active(SOCK_DGRAM, suffix, -1, NULL, &fd_udp, dscp);


   struct sockaddr_in my_addr;
   bzero(&my_addr, sizeof(my_addr));
   int len = sizeof(my_addr);
   getsockname(fd_udp, (struct sockaddr *)&my_addr, &len);
   VLOG_WARN("Port in normal %d\n", my_addr.sin_port);
   VLOG_WARN("Port in htons %d with socket %d\n", htons(my_addr.sin_port), fd_udp);

   if(fd_udp >= 0) {
      return new_udp_stream(xstrdup(name), fd_udp, error, streamp);
   } else {
      VLOG_ERR("%s: connect: %s", name, ovs_strerror(error));
      return error;
   }
